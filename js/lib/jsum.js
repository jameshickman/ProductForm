...
const init_hydration_lifecycle = () => {
    ...
    for (const el_component of els_components) {
        ...
        observedElements.add(el_component);
    }
}
...
/**
 * Clean up IntersectionObserver for elements that are removed from DOM
 */
const cleanup_observer = () => {
    observedElements.forEach(el => {
        if (!document.contains(el)) {
            observer.unobserve(el);
            observedElements.delete(el);
            if (el.id) {
                delete hydration[el.id];
            }
        }
    });
};

export {multicall, clear_cache_for, init_hydration_lifecycle, cleanup_observer};